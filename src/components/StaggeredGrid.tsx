...
export function StaggeredGrid({
    ...
    const mixedGridItems: (string | 'BENTO_GROUP')[] = Array.from({ length: 21 }, (_, i) => images[i % (images.length || 1)] || '');
    mixedGridItems[16] = 'BENTO_GROUP';

    return (
        <div
            ref={containerRef}
            className={cn("shadow relative overflow-hidden w-full pb-16", className)}
        >
            <section className="grid place-items-center w-full relative mt-[10vh] mb-[5vh]">
                <div className="text font-alt uppercase flex content-center text-[clamp(3rem,14vw,10rem)] leading-[0.7] text-neutral-900 dark:text-white">
                    {splitText(centerText)}
                </div>
            </section>

            <section className="grid place-items-center w-full relative">
                <div className="grid--full relative w-full my-[5vh] h-auto aspect-[1.1] max-w-none p-4 grid gap-4 grid-cols-7 grid-rows-5">
                        ...
                        if (typeof item === 'string') {
                            ...
                            return (
                                <motion.figure 
                                    ...
                                    transition={{ duration: 0.8, delay: (i % 7) * 0.1 }}
                                ...
                                >
                                    <div className="grid__item-img w-full h-full [backface-visibility:hidden] will-change-transform rounded-xl overflow-hidden shadow-sm border border-zinc-200 dark:border-zinc-900 bg-zinc-100 dark:bg-zinc-950 flex items-center justify-center transition-all duration-500 ease-out group-hover:scale-105 group-hover:shadow-xl group-hover:border-transparent">
                                        <img src={item} loading="lazy" alt="" className="absolute inset-0 w-full h-full object-cover opacity-20" />
